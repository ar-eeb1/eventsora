...
const BookingDetails = () => {
  ...
  useEffect(() => {
    setStatusOptions(paymentStatus.map(s => ({ label: s.charAt(0).toUpperCase() + s.slice(1), value: s })))
  }, [])

  const handleStatusUpdate = async (type, newValue) => {
    try {
      const endpoint = type === 'payment'
        ? `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/bookings/status`
        : null; // Only payment for admin
      ...
      if (data.success) {
        ...
        setBooking(prev => ({ ...prev, [type === 'payment' ? 'status' : 'bookingStatus']: newValue }))
      } else {
              ...
              <CardContent className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    setSelected={(val) => handleStatusUpdate('payment', val)}
