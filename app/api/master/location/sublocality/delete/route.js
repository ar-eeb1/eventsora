...
export async function PUT(request) {
    try {
        ...
        const sublocality = await SublocalityModel.find({ _id: { $in: ids } }).lean()
        ...
        if (deleteType === 'SD') {
            await SublocalityModel.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: new Date().toISOString() } })
        } else {
            await SublocalityModel.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: null } })
        }
...
export async function DELETE(request) {
    ...
    try {
        ...
        await SublocalityModel.deleteMany({ _id: { $in: ids } })
