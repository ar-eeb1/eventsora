...
export async function PUT(request) {
    try {
        ...
        const city = await CityModel.find({ _id: { $in: ids } }).lean()
        if (!city.length) {
        ...
        if (deleteType === 'SD') {
            await CityModel.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: new Date().toISOString() } })
        } else {
            await CityModel.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: null } })
        }
...
export async function DELETE(request) {
    ...
    try {
        ...
        await CityModel.deleteMany({ _id: { $in: ids } })
