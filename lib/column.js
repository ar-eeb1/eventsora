...
export const DT_LISTING_COLUMN = [
    ...
    },
    {
        ...
        Cell: ({ renderedCellValue }) => {
            if (!renderedCellValue || renderedCellValue.length === 0) return '-'
            return (
                <div className="flex flex-wrap gap-1">
                    {renderedCellValue.map((tag) => (
                        <span key={tag} className="text-[10px] bg-pink-100 text-pink-700 px-1.5 py-0.5 rounded border border-pink-200 font-semibold truncate capitalize">
                            {tag.replace(/_/g, ' ')}
                        </span>
                    ))}
                </div>
